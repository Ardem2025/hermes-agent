                ...
                if text_to_send:
                    ...
                    if future is None:
                        ...
                        if send_result and not getattr(send_result, "success", True):
                            ...
                            is_tg_thread_err = False
                            if platform_name.lower() == "telegram" and thread_id:
                                err_lower = str(err).lower()
                                if (
                                    "requires a reply anchor" in err_lower
                                    or "thread not found" in err_lower
                                    or "reply message not found" in err_lower
                                    or "message thread not found" in err_lower
                                ):
                                    is_tg_thread_err = True

                            if is_tg_thread_err:
                                logger.info(
                                    "Job '%s': live adapter send with thread_id=%s failed (%s), retrying without thread_id",
                                    job["id"], thread_id, err,
                                )
                                from agent.async_utils import safe_schedule_threadsafe
                                future = safe_schedule_threadsafe(
                                    runtime_adapter.send(chat_id, text_to_send, metadata=None),
                                    loop,
                                )
                                if future is not None:
                                    ...
                                    if send_result and getattr(send_result, "success", True):
                                        logger.info(
                                            "Job '%s': successfully delivered to %s:%s without thread_id",
                                        ...
                                        err = getattr(send_result, "error", "unknown")
                                        logger.warning(
                                            "Job '%s': retry without thread_id failed: %s",
                                            job["id"], err,
                                        )
                                        adapter_ok = False
                                else:
        ...
        if not delivered:
            ...
            try:
                ...
                if result and result.get("error"):
                    ...
                    if platform_name.lower() == "telegram" and thread_id and (
                        "requires telegram_reply_to_message_id" in err_msg
                        or "thread not found" in err_msg
                        or "reply message not found" in err_msg
                        or "message thread not found" in err_msg
                    ):
                        retry_without_thread = True
            except Exception as e:
                err_msg = str(e).lower()
                if platform_name.lower() == "telegram" and thread_id and (
                    "requires telegram_reply_to_message_id" in err_msg
            ...
            if retry_without_thread:
                logger.info(
                    "Job '%s': standalone send with thread_id=%s failed, retrying without thread_id",
                    job["id"], thread_id,
                )
                try:
                    coro = _send_to_platform(platform, pconfig, chat_id, cleaned_delivery_content, thread_id=None, media_files=media_files)
                    try:
                        result = asyncio.run(coro)
                    except RuntimeError:
                        coro.close()
                        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
                            future = pool.submit(asyncio.run, _send_to_platform(platform, pconfig, chat_id, cleaned_delivery_content, thread_id=None, media_files=media_files))
                            result = future.result(timeout=30)
                except Exception as e:
                    msg = f"delivery to {platform_name}:{chat_id} failed (fallback without thread_id): {e}"
                    logger.error("Job '%s': %s", job["id"], msg)
                    delivery_errors.append(msg)
                    continue
