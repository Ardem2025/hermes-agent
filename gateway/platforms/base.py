...
def _media_delivery_allowed_roots() -> List[Path]:
    ...
    # Gather base hermes home directories (profile-root and master-root)
    base_roots = [_HERMES_HOME]
    if "profiles" in _HERMES_HOME.parts:
        try:
            idx = _HERMES_HOME.parts.index("profiles")
            base_roots.append(Path(*_HERMES_HOME.parts[:idx]))
        except Exception:
            pass
    default_root = Path("/root/.hermes")
    try:
        if default_root.exists() and default_root not in base_roots:
            base_roots.append(default_root)
    except PermissionError:
        pass

    # For each base root, construct the safe cache structure
    for br in base_roots:
        roots.extend([
            ...
            br / "video_cache",
            br / "document_cache",
            br / "browser_screenshots",
            br / "cache" / "images",
            br / "cache" / "audio",
            br / "cache" / "videos",
            br / "cache" / "documents",
            br / "cache" / "screenshots",
        ])

    for r in MEDIA_DELIVERY_SAFE_ROOTS:
        if r not in roots:
            roots.append(Path(r))
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            ...
            if response:
                ...
                # Inform Dmitry if we are about to respond to a Telegram Business message
                if (
                    getattr(event.source, "telegram_business_connection_id", None)
                    and event.source.chat_id != "139351986"
                ):
                    try:
                        client_name = event.source.user_name or event.source.chat_name or "Клиент"
                        client_id = event.source.chat_id
                        client_msg = event.text or "[Медиа/Команда]"
                        
                        info_message = (
                            f"🔔 *Telegram Business ассистент*\n"
                            f"Планирую ответить пользователю *{client_name}* (ID: `{client_id}`):\n\n"
                            f"💬 *Сообщение клиента:*\n{client_msg}\n\n"
                            f"🤖 *Мой планируемый ответ:*\n{text_content}"
                        )
                        await self.send(
                            chat_id="139351986",
                        ...
                        logger.warning("[%s] Failed to send Business notification to owner: %s", self.name, inf_err)
