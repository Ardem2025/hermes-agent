...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _thread_kwargs_for_send(
        ...
        if metadata and metadata.get("business_connection_id"):
            kwargs["business_connection_id"] = metadata["business_connection_id"]

        if metadata and metadata.get("telegram_dm_topic_reply_fallback"):
            if reply_to_mode == "off":
                kwargs["message_thread_id"] = cls._message_thread_id_for_send(thread_id)
                return kwargs
                    ...
                    try:
                        ...
                        if _BadReq and isinstance(send_err, _BadReq):
                            if self._is_thread_not_found_error(send_err) and effective_thread_id is not None:
                                is_private_thread = private_dm_topic_send or (metadata and metadata.get("telegram_dm_topic_created_for_send"))
                                if not retried_thread_not_found:
                                    retried_thread_not_found = True
                                    if not is_private_thread:
                                        logger.warning(
                            ...
                            if "message to be replied not found" in err_lower and reply_to_id is not None:
                                is_private_reply = bool(private_dm_topic_send)
                                if not is_private_reply:
                                    logger.warning(
                                        "[%s] Reply target deleted, retrying without reply_to: %s",
                                        self.name, send_err,
                                    )
                                reply_to_id = None
                                if is_private_reply or (metadata and metadata.get("telegram_dm_topic_reply_fallback")):
                                    thread_kwargs = {"message_thread_id": None}
                                    effective_thread_id = None
                                else:
                                    thread_kwargs = self._thread_kwargs_for_send(
    ...
    async def edit_message(
        ...
        *,
        ...
        edit_kwargs = {}
        if metadata and metadata.get("business_connection_id"):
            edit_kwargs["business_connection_id"] = metadata["business_connection_id"]
            ...
            if cached_tid == thread_id_int and key.startswith(f"{chat_id}:"):
                ...
                for chat_entry in self._dm_topics_config:
                    if str(chat_entry.get("chat_id")) == chat_id:
                        for t in chat_entry.get("topics", []):
                            if t.get("name") == topic_name:
                                return dict(t, operator_declared=True)
                return {"name": topic_name, "operator_declared": False}
    ...
    def _build_message_event(
        ...
        if getattr(message, "business_connection_id", None):
            source.telegram_business_connection_id = message.business_connection_id
