...
def claim_task(
    ...
    *,
    ...
    with write_txn(conn):
        # Child-profile safety rail: the shared Kanban board is a coordination
        # primitive, not a permission boundary. After the 2026-06-07 incident
        # where profile "nestor" executed production dvapara.ru SEO tasks,
        # enforce tenant/profile isolation at the final claim point. This keeps
        # accidental assignee/tenant combinations from spawning a child worker.
        if claimer == "nestor" or os.environ.get("HERMES_PROFILE") == "nestor":
            tenant_row = conn.execute(
                "SELECT tenant, assignee FROM tasks WHERE id = ? AND status = 'ready'",
                (task_id,),
            ).fetchone()
            tenant = (tenant_row["tenant"] if tenant_row else None) or ""
            assignee = (tenant_row["assignee"] if tenant_row else None) or ""
            allowed_tenants = {
                t.strip()
                for t in os.environ.get("HERMES_NESTOR_ALLOWED_KANBAN_TENANTS", "nestor,Nestor,spooky,Spooky,game,roblox").split(",")
                if t.strip()
            }
            if tenant and tenant not in allowed_tenants:
                ...
                _append_event(
                    ...
                    "claim_rejected",
                    {
                        "reason": "tenant_not_allowed_for_nestor",
                        "tenant": tenant,
                        "assignee": assignee,
