...
class AIAgent:
    ...
    def _persist_session(self, messages: List[Dict], conversation_history: List[Dict] = None):
        ...
        """
        self._drop_trailing_empty_response_scaffolding(messages, conversation_history)
        self._apply_persist_user_message_override(messages)
        self._session_messages = messages
        self._save_session_log(messages)
        self._flush_messages_to_session_db(messages, conversation_history)

    def _drop_trailing_empty_response_scaffolding(
        self,
        messages: List[Dict],
        ...
        """
        # If the run was interrupted or failed mid-turn (due to user stop, provider timeouts,
        # or OmniRoute/OpenRouter API errors), we must clean up the trailing incomplete 
        # assistant thoughts and tool results generated during this turn to prevent thought leaks
        # when the user resumes or sends a new prompt.
        is_failed_or_interrupted = False
        if interrupted or failed or (hasattr(self, "_interrupt_requested") and self._interrupt_requested):
            is_failed_or_interrupted = True
        
        # If we have a stable conversation history, we can safely reset messages back
        # to the state right after the user triggered this turn (history + initial user message).
        if is_failed_or_interrupted and conversation_history is not None:
            stable_len = len(conversation_history)
            # Find the user's triggering message for this active turn
            if len(messages) > stable_len and messages[stable_len].get("role") == "user":
                stable_len += 1
            # Discard any incomplete assistant/tool messages appended during this failed turn
            while len(messages) > stable_len:
                messages.pop()
