...
install_deps() {
    ...
    if [ -f "uv.lock" ]; then
        ...
        local _CORE2DUO_CONSTRAINT_ARGS=()
        if [ -f "$INSTALL_DIR/constraints-core2duo.txt" ]; then
            _CORE2DUO_CONSTRAINT_ARGS=(-c "$INSTALL_DIR/constraints-core2duo.txt")
        fi
        if UV_PROJECT_ENVIRONMENT="$INSTALL_DIR/venv" $UV_CMD sync --extra all --locked "${_CORE2DUO_CONSTRAINT_ARGS[@]}"; then
...
PY
    ...
    install_tier() {
        ...
        if $UV_CMD pip install "${_CORE2DUO_CONSTRAINT_ARGS[@]}" -e "$spec" 2>"$ALL_INSTALL_LOG"; then
