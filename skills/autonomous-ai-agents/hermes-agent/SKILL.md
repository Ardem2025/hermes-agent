...
⚠️ **CRITICAL SECURITY NOTE**: Do NOT attempt to directly modify, write, or patch files under `~/.hermes/` (such as `config.yaml`, `.env`, or the `state.db` SQLite store) using raw file tools like `patch_ide` or `write_file_ide`. These path patterns are blocklisted by the platform's **File-mutation verifier** as a safeguard to secure user credentials and config schemas. Raw modifications will fail with a `protected system/credential file` error. Always modify Hermes settings using the dedicated CLI tool: `hermes config set <key> <val>` inside `terminal_ide`.
