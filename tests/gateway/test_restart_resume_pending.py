...
@pytest.mark.asyncio
async def test_restart_banner_uses_custom_wording():
    """The notification sent before drain should match the customized wording."""
    runner, adapter = make_restart_runner()
    runner._restart_requested = True
    ...
    msg = adapter.sent[0]
    assert msg == "Внимание! Для активации новых функций и/или обслуживания система перезапустила ассистента. Вы можете продолжать работу."
...
async def test_restart_notifies_home_channel_even_without_active_sessions():
    ...
    assert adapter.sent == [
        "Внимание! Для активации новых функций и/или обслуживания система перезапустила ассистента. Вы можете продолжать работу."
    ]
