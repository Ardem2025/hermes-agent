...
def test_user_created_telegram_topic_is_lane_without_topic_mode_flag():
    """A real DM thread must be title-eligible even before /topic activation."""
    runner = _make_runner()
    runner._telegram_topic_mode_enabled = lambda source: False

    assert runner._is_telegram_topic_lane(_make_source(thread_id="42")) is True
    assert runner._is_telegram_topic_lane(_make_source(thread_id=None)) is False
    assert runner._is_telegram_topic_lane(_make_source(thread_id="1")) is False
...
def test_migration_rebuilds_v1_binding_table_with_cascade_fk(tmp_path):
    ...
    # Re-run migration — should upgrade through v2 CASCADE to the v3 delivery shape.
    db.apply_telegram_topic_migration()
