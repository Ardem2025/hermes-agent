...
def test_drop_scaffolding_interrupted():
    """When interrupted flag is True, scaffolding is stripped even without _interrupt_requested."""
    agent = _bare_agent()
    messages = [
        {"role": "user", "content": "task"},
        {"role": "assistant", "content": "",
    ...
    AIAgent._drop_trailing_empty_response_scaffolding(agent, messages, interrupted=True)
    assert messages == [{"role": "user", "content": "task"}]


def test_drop_scaffolding_failed():
    """When failed flag is True, scaffolding is stripped (covers provider crashes)."""
    agent = _bare_agent()
    messages = [
        {"role": "user", "content": "task"},
        {"role": "assistant", "content": "",
    ...
    AIAgent._drop_trailing_empty_response_scaffolding(agent, messages, failed=True)
    assert messages == [{"role": "user", "content": "task"}]
