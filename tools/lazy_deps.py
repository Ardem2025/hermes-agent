...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    "stt.faster_whisper": (
        ...
        "sounddevice==0.5.5",
        # kuzhomesrv/Core2Duo lacks AVX/SSE4.x; numpy 2.4.x wheels SIGILL.
        "numpy==1.26.4",
    ),
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    constraints = Path("/opt/hermes-agent/constraints-core2duo.txt")
    constraint_args = ["-c", str(constraints)] if constraints.exists() else []
    ...
    if uv_bin:
        try:
            r = subprocess.run(
                [uv_bin, "pip", "install", *constraint_args, *specs],
